//! 同步状态详情页：pending 队列、最近 op、游标与错误信息。
//! 复用 HeroUI Modal（焦点陷阱 / Esc / 滚动锁定与其余弹窗一致）；
//! compact 上为底部抽屉，否则居中弹层。
...
function OpRow({ op, dir }: { op: SyncOpInfo; dir?: "pending" | "recent" }) {
  ...
  return (
    <li className="flex items-start gap-2 rounded-lg px-2 py-1.5 text-xs hover:bg-pw-hover">
      <span
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-fg">{describePatch(op.patch)}</span>
        <span className="text-fg-subtle">
          {dir === "recent"
            ? timeLabel(op.at_ms ?? op.client_time_ms)
            : timeLabel(op.client_time_ms)}
          {" · λ"}
          {op.lamport}
...
export function SyncStatusSheet() {
  ...
  return (
    <Modal
      isOpen
      onOpenChange={(o) => {
        if (!o) actions.closeSyncSheet();
      }}
    >
      <Modal.Backdrop>
        <Modal.Container placement={bottom ? "bottom" : "center"}>
          <Modal.Dialog data-testid="sync-sheet" className="w-full max-w-md">
            <Modal.Header className="flex items-center justify-between">
              <Modal.Heading className="text-lg font-bold">同步状态</Modal.Heading>
              <button
                onClick={() => actions.closeSyncSheet()}
                className="rounded-lg px-2 py-1 text-sm text-fg-subtle hover:bg-pw-hover"
                aria-label="关闭"
            ...
            <Modal.Body className="max-h-[70vh] overflow-y-auto">
              {/* 概览 */}
              <div className="mb-4 space-y-1.5 rounded-xl bg-pw-surface-2 p-3 text-xs text-fg-muted">
              ...
              <section className="mb-4">
                ...
                {details && details.pending.ops.length > 0 ? (
                  ...
                  <div className="rounded-lg bg-pw-surface-2 px-3 py-3 text-center text-xs text-fg-subtle">
                    没有待同步的更改
                  </div>
                )}
              </section>
              ...
              <section className="mb-1">
                <h3 className="mb-1 text-xs font-medium text-fg-subtle">最近操作</h3>
