 ...
 * 点条目 → 冒泡关浮层（`TaskRow` 自身已打开详情）；底部按钮 = 该日新建入口。
 */
export function DayTasksOverlay(props: {
  ...
  return (
    ...
    >
      <Modal.Backdrop>
        <Modal.Container placement={narrow ? "bottom" : "center"}>
          <Modal.Dialog data-testid="day-tasks-overlay" className="w-full sm:max-w-md">
            <Modal.Header>
              <Modal.Heading className="text-base font-bold">{props.title}</Modal.Heading>
            </Modal.Header>
            <Modal.Body className="flex flex-col gap-3">
              {props.tasks.length === 0 ? (
                <p className="py-6 text-center text-sm text-zinc-400">这天没有任务</p>
              ) : (
