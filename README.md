...
│   └── server/       # Axum 后端：/auth、/sync/push|pull、sqlx + MySQL
├── crates/
...
5. 客户端按 `last_pulled_seq` 增量 `GET /sync/pull?since=`，分页拉取直至追平；
6. 触发时机全部收敛到同一个 `refresh()`（push + pull）：应用启动、窗口聚焦/前台切换、前台 60s 轮询、每次 push 成功后，以及顶栏手动刷新按钮；断网时 push 失败仅保留队列，下次刷新重试。

**收敛与冲突**

7. op 的 patch 是**字段级**的：不同字段的并发修改互不覆盖；
8. 同字段并发由全序决胜（field-level LWW）：所有端按同一 `seq` 全序重放，结果必然一致；
9. 删除是软删除墓碑（`deleted: true`），对墓碑的其他字段编辑不会复活它，只有显式 `deleted: false` 才会；
10. Lamport 时钟随拉取推进，保证客户端本地 op 的时钟不低于已见过的任何 op。
...
| 层 | 内容 | 位置 |
|---|---|---|
| 同步内核 | 定序重放、幂等去重、字段独立性、墓碑语义、乱序到达、LWW 终态定义、随机化收敛、客户端状态机（队列/单飞/尾随合并）、与前端 JSON 契约 | `crates/sync-core`（29 例） |
| 服务端 | 注册/登录/刷新/鉴权失败路径、全序分配、增量拉取、重复投递幂等、批量原子性 | `apps/server`（9 例；MySQL 用例无 `DATABASE_URL` 时自动跳过） |
...
- 下拉刷新手势（移动端）、同步状态详情页（查看 pending 队列与最近 op）
