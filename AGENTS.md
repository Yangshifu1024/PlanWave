...
- Native builds need a prior `tauri android dev|build` to generate `gen/android/app/src/main/java/<package>/generated/*.kt` (gitignored); a bare `gradlew` task fails with `Unresolved reference: TauriActivity` otherwise. After an identifier rename, also delete the stale `app/src/main/java/<old-package>/` tree — it is untracked but still compiled, and breaks the build with `Unresolved reference: BuildConfig`.
- Real-device Android debugging: `tauri android dev` rewrites the devUrl host to the LAN IP and injects `TAURI_DEV_HOST` before running `beforeDevCommand`, so `vite.config.ts` binds all interfaces when that variable is present (plain `pnpm dev:web` stays on localhost). Point the phone at the dev API over USB with `adb reverse tcp:8787 tcp:8787` so the default `http://127.0.0.1:8787` works without editing the server address on the login screen. The reverse mapping does not survive an adb server restart, and a release-signed install blocks the debug APK with `INSTALL_FAILED_UPDATE_INCOMPATIBLE` — uninstall it first (that wipes the app's local data).
